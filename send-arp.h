...
void send_arp(pcap_t* handle,const char* dmac,const char* smac, const char* sip, const char* tmac, const char* tip,int mode)
{
    ...
    if(mode == ARP_REQ) packet.arp_.op_ = htons(ArpHdr::Request);
    else if(mode == ARP_REP) packet.arp_.op_ = htons(ArpHdr::Reply);
...
char* get_mac_by_arp(pcap_t* handle,const char* a_mac,const char* a_ip,const char * t_ip,int mode)
{
	...
	while(true)
	{
		...
		if(packet != NULL)
        {
            ...
            if(chk_a_ip == arp.tip() && chk_t_ip == arp.sip() && chk_a_mac == arp.tmac()) break;
        }
	}
...
{
    struct ifreq ifr;
    static __thread char ip[IP_LEN+1];
